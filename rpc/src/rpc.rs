...
impl JsonRpcRequestProcessor {
    ...
    pub async fn get_account_info(
        ...
        let response = self
            ...
            .spawn_blocking({
                ...
                move || get_encoded_account(&bank, &pubkey, encoding, data_slice)
            })
            .await
    ...
    pub async fn get_program_accounts(
        ...
        let keyed_accounts = {
            if let Some(owner) = get_spl_token_owner_filter(&program_id, &filters) {
                ...
                self.get_filtered_program_accounts(Arc::clone(&bank), program_id, filters)
                    .await?
            }
        };
    ...
    pub async fn get_token_largest_accounts(
        ...
        let mut token_balances: Vec<RpcTokenAccountBalance> = self
            .get_filtered_spl_token_accounts_by_mint(Arc::clone(&bank), mint_owner, mint, vec![])
            .await?
            .into_iter()
            .map(|(address, account)| {
    ...
    pub async fn get_token_accounts_by_delegate(
        ...
        let keyed_accounts = if let Some(mint) = mint {
            ...
            self.get_filtered_program_accounts(Arc::clone(&bank), token_program_id, filters)
                .await?
        };
        let accounts = if encoding == UiAccountEncoding::JsonParsed {
            get_parsed_token_accounts(bank.clone(), keyed_accounts.into_iter()).collect()
